...
class serverRPC:
	def __init__(self, ip="localhost", puerto=5500, tipo="main"):
		...
		button = tkinter.Button(frame, text='Stop Clock', command=self.stopClock)
		button.grid(row=3, columnspan=2)
		button = tkinter.Button(frame, text='Sinc Clock', command=self.sincClock)
	...
	def sincClock(self):
		if(self.conecction is not None):
			...
			self.clockTime.setTime(valueFloat=value)
		if(self.tipo!="main"):
			self.clock.after(5000, self.sincClock)
	...
	def runServer(self):
		...
		if self.tipo=="main":
			...
			self.conecction=xmlrpc.client.ServerProxy("http://"+ipServer+":"+puertoServer)
